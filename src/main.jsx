...
import { ArrowUpRight, Boxes, Check, Download, ExternalLink, Code2, Globe, Menu, Smartphone, Sparkles, UploadCloud, X, Trash2, LockKeyhole, FileArchive } from 'lucide-react'
import './styles.css'
  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section className="about" id="about"><div className="about-card"><div><p className="section-kicker">A living showcase</p><h2>Built to be updated.</h2><p>Add live projects or upload APK, EXE and browser extension files from the Manage panel. Uploaded files are kept in private Blob storage and delivered through a controlled download endpoint.</p></div><div className="about-code"><span>project</span><strong>{'{ url | apk | exe | extension }'}</strong></div></div></section>
      </main>
...
function AdminPanel({ projects, setProjects, adminKey, setAdminKey, message, setMessage, onClose }) {
  ...
  async function saveProject(event) {
    ...
    try {
      ...
      const response = await fetch('/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-key': key }, body: JSON.stringify({ projects: [project, ...projects] }) })
