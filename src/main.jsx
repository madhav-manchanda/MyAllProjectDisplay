...
function App() {
  ...
  useEffect(() => { fetch('/api/projects').then((r) => r.ok ? r.json() : fallbackProjects).then(setProjects).catch(() => {}) }, [])
...
function AdminPanel({ projects, setProjects, adminKey, setAdminKey, message, setMessage, onClose }) {
  const fileRef = useRef(null); const [form, setForm] = useState({ name: '', type: 'Live', description: '', version: '', url: '', github: '', featured: false }); const [file, setFile] = useState(null); const [busy, setBusy] = useState(false)
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="admin-panel"><div className="admin-head"><div><p className="section-kicker">Private controls</p><h2>Manage projects</h2></div><button className="icon-button" onClick={onClose}><X size={18} /></button></div><p className="admin-note">Your admin key is sent only to the Vercel API to authorize uploads and project changes. It is never saved in the project files.</p><label className="field"><span>Admin key</span><input type="password" value={adminKey} onChange={(e) => setAdminKey(e.target.value)} placeholder="Vercel ADMIN_UPLOAD_KEY" /></label><form onSubmit={saveProject} className="admin-form"><div className="form-row"><label className="field"><span>Project name</span><input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="My Project" /></label><label className="field"><span>Type</span><select value={form.type} onChange={(e) => update('type', e.target.value)}><option>Live</option><option>APK</option></select></label></div><label className="field"><span>Short description</span><textarea value={form.description} onChange={(e) => update('description', e.target.value)} placeholder="What does this project do?" rows="3" /></label><div className="form-row"><label className="field"><span>Version / platform</span><input value={form.version} onChange={(e) => update('version', e.target.value)} placeholder="Web app / Android app" /></label><label className="field"><span>GitHub URL <em>optional</em></span><input value={form.github} onChange={(e) => update('github', e.target.value)} placeholder="https://github.com/..." /></label></div>{form.type === 'Live' ? <label className="field"><span>Deployed URL</span><input value={form.url} onChange={(e) => update('url', e.target.value)} placeholder="https://my-project.vercel.app" /></label> : <label className="upload-box" htmlFor="apk-upload"><UploadCloud size={30} /><strong>{file ? file.name : 'Upload Android APK'}</strong><span>{file ? `${(file.size / 1024 / 1024).toFixed(1)} MB selected` : 'Click here to choose an .apk file'}</span><input id="apk-upload" ref={fileRef} type="file" accept=".apk,application/vnd.android.package-archive,application/octet-stream" onChange={(e) => setFile(e.target.files?.[0] || null)} /></label>}<label className="check-row"><input type="checkbox" checked={form.featured} onChange={(e) => update('featured', e.target.checked)} /><span>Mark as featured</span></label><button className="save-button" disabled={busy}>{busy ? 'Working…' : <><Check size={16} /> Add project</>}</button></form>{message && <div className="admin-message">{message}</div>}<div className="admin-list"><p className="section-kicker">Current projects</p>{projects.map((project) => <div className="admin-project" key={project.id}><div><strong>{project.name}</strong><span>{project.type}</span></div><button disabled={busy} onClick={() => removeProject(project.id)} title={`Delete ${project.name}`}><Trash2 size={15} /></button></div>)}</div></aside></div>
}
